  ...
  <div class="inner">
    ...
    <div id="adminPanel">
      ...
      <div class="admin-pane" id="pane-route">
        ...
        <p style="font-size:.82rem;color:var(--muted);margin-bottom:.8rem">Edit, reorder or remove events. Each event is a checkpoint on the animated map.</p>
      ...
      <div class="admin-pane" id="pane-members">
        ...
        <p style="font-size:.82rem;color:var(--muted);margin-bottom:.8rem">Click ✏️ Edit to update any member's name, role, awards, or avatar.</p>
      ...
      <div class="admin-pane" id="pane-stats">
        ...
        <p style="font-size:.82rem;color:var(--muted);margin-bottom:1rem">These numbers animate on the homepage hero section.</p>
        <div class="stats-row">
...
function save(k,v){try{localStorage.setItem(k,JSON.stringify(v));}catch(e){toast('⚠️ Storage full!','err');}}

let certs   = load('apex_certs',   []);
let stats   = load('apex_stats',   DEFAULT_STATS);
let members = load('apex_members', DEFAULT_MEMBERS);
let route   = load('apex_route',   DEFAULT_ROUTE);
...
function saveStats(){
  ...
  save('apex_stats',stats);renderStats();toast('✅ Stats saved!','ok');
}
...
function addCert(){
  ...
  save('apex_certs',certs);renderCerts();
...
function editCert(id){
  ...
  openEditModal('✏️ Edit Certificate',`
    ...
    save('apex_certs',certs);renderCerts();closeEditModal();toast('✅ Certificate updated!','ok');
  });
}
function deleteCert(id){if(!confirm('Delete this certificate?'))return;certs=certs.filter(c=>c.id!==id);save('apex_certs',certs);renderCerts();toast('🗑 Deleted','info');}
function confirmClearAll(){if(!confirm('Delete ALL certificates?'))return;certs=[];save('apex_certs',certs);renderCerts();toast('🗑 All cleared','info');}
...
function renderRouteList(){
  ...
  if(!route.length){list.innerHTML='<p style="color:var(--muted);font-size:.85rem;padding:.5rem 0">No events yet — add one below.</p>';return;}
  list.innerHTML='';
  route.forEach((r,i)=>{
    ...
    row.innerHTML=`
      ...
      <div class="er-info">
        <div class="er-title">${esc(r.label)}</div>
        <div class="er-sub">${esc(r.event)} &nbsp;·&nbsp; ${esc(r.rank)}</div>
      </div>
      <div class="er-btns">
...
function addRouteEvent(){
  ...
  save('apex_route',route);renderRouteList();
  ['rlLabel','rlEvent','rlRank'].forEach(id=>document.getElementById(id).value='');
  toast('✅ Event added to route!','ok');
}
  ...
  openEditModal('✏️ Edit Route Event',`
    <div class="admin-grid">
      ...
      <div class="field-group"><label>Dot Color</label>
        <select id="er_color">
          <option value="#00E5FF" ${r.color==='#00E5FF'?'selected':''}>🔵 Cyan — Start / Neutral</option>
          <option value="#FFD600" ${r.color==='#FFD600'?'selected':''}>🟡 Gold — Win</option>
          <option value="#FF3D00" ${r.color==='#FF3D00'?'selected':''}>🔴 Red — Finish / Major</option>
          <option value="#22c55e" ${r.color==='#22c55e'?'selected':''}>🟢 Green — Participation</option>
        </select>
      </div>
    </div>`,()=>{
    ...
    save('apex_route',route);renderRouteList();closeEditModal();toast('✅ Event updated!','ok');
  });
}
function deleteRouteEvent(id){if(!confirm('Remove this route event?'))return;route=route.filter(r=>r.id!==id);save('apex_route',route);renderRouteList();toast('🗑 Removed','info');}
function moveRoute(idx,dir){
  const ni=idx+dir;if(ni<0||ni>=route.length)return;
  [route[idx],route[ni]]=[route[ni],route[idx]];save('apex_route',route);renderRouteList();
}
...
function renderMemberList(){
  ...
  members.forEach(m=>{
    ...
    row.innerHTML=`
      ...
      <div class="er-info">
        <div class="er-title">${esc(m.name)}</div>
        <div class="er-sub">${esc(m.role)} &nbsp;·&nbsp; ${esc(m.wins)} awards</div>
      </div>
      <div class="er-btns">
...
function renderTeamGrid(){
  ...
  if(!members.length){grid.innerHTML='<p style="color:var(--muted);font-family:\'Barlow Condensed\',sans-serif;letter-spacing:.1em">No members yet — add some in Admin → Members.</p>';return;}
  members.forEach(m=>{
...
function addMember(){
  ...
  save('apex_members',members);renderMemberList();renderTeamGrid();
...
function editMember(id){
  ...
  openEditModal('✏️ Edit Member',`
    <div class="admin-grid">
      ...
      <div class="field-group"><label>Awards Won</label><input type="text" id="em_wins" value="${esc(m.wins)}" placeholder="3 or —"/></div>
      <div class="field-group"><label>Avatar Letter</label><input type="text" id="em_letter" value="${esc(m.letter)}" maxlength="2"/></div>
    </div>
    <div class="field-group" style="margin-bottom:.5rem;max-width:340px">
      <label>Avatar Color</label>
      <select id="em_color">${opts}</select>
    </div>`,()=>{
    ...
    save('apex_members',members);renderMemberList();renderTeamGrid();closeEditModal();toast('✅ Member updated!','ok');
  });
}
function deleteMember(id){if(!confirm('Remove this member?'))return;members=members.filter(m=>m.id!==id);save('apex_members',members);renderMemberList();renderTeamGrid();toast('🗑 Removed','info');}
...
function rmResize(){
  const w=rmCanvas.parentElement;
  rmW=rmCanvas.width=w.clientWidth*devicePixelRatio;
  rmH=rmCanvas.height=w.clientHeight*devicePixelRatio;
  rmCanvas.style.width=w.clientWidth+'px';rmCanvas.style.height=w.clientHeight+'px';
  rmBuildPath();rmDraw();
}
function rmBuildPath(){
  const n=route.length;
  rmPts=route.map((_,i)=>{const t=i/(n-1||1);return{x:(i%2===0)?rmW*.18:rmW*.80,y:rmH*.07+t*(rmH*.86)};});
  rmPath=[];const S=1400;
  for(let s=0;s<=S;s++){
    const t=s/S,idx=t*(rmPts.length-1),i1=Math.floor(idx);
    const i0=Math.max(0,i1-1),i2=Math.min(rmPts.length-1,i1+1),i3=Math.min(rmPts.length-1,i1+2);
    rmPath.push(rmCatmull(rmPts[i0],rmPts[i1],rmPts[i2],rmPts[i3],idx-i1));
  }
  for(let i=0;i<rmPath.length;i++){
    const nx=rmPath[Math.min(i+1,rmPath.length-1)],pv=rmPath[Math.max(i-1,0)];
    rmPath[i].angle=Math.atan2(nx.y-pv.y,nx.x-pv.x);
  }
}
function rmCatmull(p0,p1,p2,p3,t){
  const t2=t*t,t3=t2*t;
  return{x:.5*((-p0.x+3*p1.x-3*p2.x+p3.x)*t3+(2*p0.x-5*p1.x+4*p2.x-p3.x)*t2+(-p0.x+p2.x)*t+2*p1.x),
         y:.5*((-p0.y+3*p1.y-3*p2.y+p3.y)*t3+(2*p0.y-5*p1.y+4*p2.y-p3.y)*t2+(-p0.y+p2.y)*t+2*p1.y)};
}
function rmDraw(){
  rmCtx.clearRect(0,0,rmW,rmH);const dpr=devicePixelRatio;
  rmCtx.save();rmCtx.strokeStyle='rgba(255,255,255,.04)';rmCtx.lineWidth=1;
  const G=60*dpr;
  for(let x=0;x<rmW;x+=G){rmCtx.beginPath();rmCtx.moveTo(x,0);rmCtx.lineTo(x,rmH);rmCtx.stroke();}
  for(let y=0;y<rmH;y+=G){rmCtx.beginPath();rmCtx.moveTo(0,y);rmCtx.lineTo(rmW,y);rmCtx.stroke();}
  rmCtx.restore();
  if(rmPath.length<2)return;
  rmCtx.save();rmCtx.lineWidth=22*dpr;rmCtx.lineCap='round';rmCtx.lineJoin='round';rmCtx.strokeStyle='rgba(255,255,255,.06)';
  rmCtx.beginPath();rmCtx.moveTo(rmPath[0].x,rmPath[0].y);for(let i=1;i<rmPath.length;i++)rmCtx.lineTo(rmPath[i].x,rmPath[i].y);rmCtx.stroke();
  rmCtx.setLineDash([14*dpr,18*dpr]);rmCtx.lineWidth=2*dpr;rmCtx.strokeStyle='rgba(255,255,255,.1)';rmCtx.stroke();rmCtx.setLineDash([]);rmCtx.restore();
  const curIdx=Math.floor(rmProgress*(rmPath.length-1));
  if(curIdx>0){
    rmCtx.save();rmCtx.lineWidth=5*dpr;rmCtx.lineCap='round';rmCtx.lineJoin='round';
    rmCtx.strokeStyle='#00E5FF';rmCtx.shadowColor='#00E5FF';rmCtx.shadowBlur=14*dpr;
    rmCtx.beginPath();rmCtx.moveTo(rmPath[0].x,rmPath[0].y);for(let i=1;i<=curIdx;i++)rmCtx.lineTo(rmPath[i].x,rmPath[i].y);rmCtx.stroke();rmCtx.restore();
  }
  route.forEach((cp,i)=>{
    const t=i/(route.length-1||1),pi=Math.round(t*(rmPath.length-1)),pp=rmPath[pi]||rmPts[i];
    const r=16*dpr,reached=rmProgress>=t-.002;
    rmCtx.save();
    if(reached){rmCtx.shadowColor=cp.color;rmCtx.shadowBlur=20*dpr;}
    rmCtx.beginPath();rmCtx.arc(pp.x,pp.y,r,0,Math.PI*2);rmCtx.strokeStyle=reached?cp.color:'rgba(255,255,255,.18)';rmCtx.lineWidth=2.5*dpr;rmCtx.stroke();
    rmCtx.beginPath();rmCtx.arc(pp.x,pp.y,r-2*dpr,0,Math.PI*2);
    rmCtx.fillStyle=reached?(i===route.length-1?'rgba(255,61,0,.35)':'rgba(0,229,255,.18)'):'rgba(10,10,15,.75)';rmCtx.fill();rmCtx.shadowBlur=0;
    rmCtx.font=`${Math.round(12*dpr)}px sans-serif`;rmCtx.textAlign='center';rmCtx.textBaseline='middle';rmCtx.fillStyle=reached?'#fff':'rgba(255,255,255,.28)';
    rmCtx.fillText(i===route.length-1?'🏁':(reached?'★':String(i)),pp.x,pp.y);
    const lx=pp.x>rmW*.5?pp.x-r-8*dpr:pp.x+r+8*dpr,align=pp.x>rmW*.5?'right':'left';
    rmCtx.font=`bold ${Math.round(11*dpr)}px 'Barlow Condensed',sans-serif`;rmCtx.textAlign=align;rmCtx.fillStyle=reached?'#F0F0F5':'rgba(255,255,255,.22)';
    rmCtx.fillText(cp.label,lx,pp.y-7*dpr);
    if(cp.rank&&reached){rmCtx.font=`${Math.round(10*dpr)}px 'Barlow Condensed',sans-serif`;rmCtx.fillStyle='#FFD600';rmCtx.fillText(cp.rank,lx,pp.y+9*dpr);}
    rmCtx.restore();
  });
  const cp=rmPath[curIdx]||rmPath[0];rmDrawCar(cp.x,cp.y,cp.angle||0);
}
function rmDrawCar(x,y,angle){
  const ctx=rmCtx,dpr=devicePixelRatio,s=2.8*dpr;
  ctx.save();ctx.translate(x,y);ctx.rotate(angle+Math.PI/2);
  ctx.shadowColor='#FF3D00';ctx.shadowBlur=22*dpr;
  ctx.fillStyle='#FF3D00';rrect(ctx,-7*s,-11*s,14*s,22*s,3*s);ctx.fill();
  ctx.fillStyle='#cc2a00';rrect(ctx,-5*s,-7.5*s,10*s,13*s,2*s);ctx.fill();
  ctx.fillStyle='rgba(0,229,255,.75)';rrect(ctx,-4*s,-6.5*s,8*s,5*s,1.2*s);ctx.fill();
  ctx.shadowBlur=0;ctx.fillStyle='#1a1a1a';
  [[-9*s,-7*s],[9*s,-7*s],[-9*s,7*s],[9*s,7*s]].forEach(([wx,wy])=>{ctx.beginPath();ctx.ellipse(wx,wy,3.5*s,2.5*s,Math.PI/2,0,Math.PI*2);ctx.fill();});
  ctx.fillStyle='#FFD600';[[-4*s,-11*s],[4*s,-11*s]].forEach(([lx,ly])=>{ctx.beginPath();ctx.arc(lx,ly,1.8*s,0,Math.PI*2);ctx.fill();});
  ctx.restore();
}
...
function rmShowCP(i){
  const cp=route[i],t=i/(route.length-1||1),pi=Math.round(t*(rmPath.length-1)),pp=rmPath[pi]||rmPts[i];
  const popup=document.getElementById('cpPopup'),wrap=rmCanvas.parentElement.getBoundingClientRect(),dpr=devicePixelRatio;
  let lx=pp.x/dpr+(pp.x>rmW*.5?-230:22),ly=pp.y/dpr-65;
  lx=Math.max(8,Math.min(lx,wrap.width-240));ly=Math.max(8,Math.min(ly,wrap.height-110));
  popup.style.left=lx+'px';popup.style.top=ly+'px';
  document.getElementById('cpEv').textContent=cp.event;document.getElementById('cpNm').textContent=cp.label;document.getElementById('cpRk').textContent=cp.rank;
  popup.classList.add('show');clearTimeout(rmCpTimer);rmCpTimer=setTimeout(()=>popup.classList.remove('show'),3500);
}
function rmLoop(ts){
  if(rmLastT===null)rmLastT=ts;const dt=(ts-rmLastT)/1000;rmLastT=ts;
  const spd=parseFloat(document.getElementById('spdSlider').value);
  rmProgress=Math.min(1,rmProgress+dt*spd*0.055);
  route.forEach((_,i)=>{const t=i/(route.length-1||1);if(rmProgress>=t&&!rmReached.has(i)){rmReached.add(i);if(i>0)rmShowCP(i);}});
  document.getElementById('progFill').style.width=(rmProgress*100).toFixed(1)+'%';
  rmDraw();
  if(rmProgress<1&&rmPlaying)rmRaf=requestAnimationFrame(rmLoop);
  else if(rmProgress>=1){rmPlaying=false;document.getElementById('rmPlayBtn').textContent='✔ Done';rmDraw();}
}
function rmTogglePlay(){
  if(rmProgress>=1){rmReset();return;}
  rmPlaying=!rmPlaying;document.getElementById('rmPlayBtn').textContent=rmPlaying?'⏸ Pause':'▶ Play';
  if(rmPlaying){rmLastT=null;rmRaf=requestAnimationFrame(rmLoop);}else cancelAnimationFrame(rmRaf);
}
function rmReset(){
  cancelAnimationFrame(rmRaf);rmProgress=0;rmPlaying=false;rmLastT=null;rmReached.clear();
  document.getElementById('rmPlayBtn').textContent='▶ Play';document.getElementById('progFill').style.width='0%';
  document.getElementById('cpPopup').classList.remove('show');rmDraw();
}
function openRoute(){
  document.getElementById('routePage').classList.add('open');document.body.style.overflow='hidden';
  setTimeout(()=>{rmResize();rmDraw();},60);
}
function closeRoute(){
  document.getElementById('routePage').classList.remove('open');document.body.style.overflow='';
  cancelAnimationFrame(rmRaf);rmPlaying=false;document.getElementById('rmPlayBtn').textContent='▶ Play';
}
